...
$(document).ready(function() {
    ...
    if (vence == null) {
        ...
        ;
        var ok = 0;
        //f.getFullYear()+''+mes+''+f.getDate());
        if (Number(fechaVen[2]) > f.getFullYear()) {
            ok = 1;
        } else {
            if (Number(fechaVen[0]) > mes) {
                //alert(fechaVen[1] +' > ' +mes);
                ok = 2;
            } else {
                if (Number(fechaVen[1]) > f.getDate()) {
                    ok = 3;
                }
            }
        }
        ...
        if (ok > 0) {
            //alert('Activa: '+vence+' >= '+hoy);
        } else {
            //alert('Vencio: '+vence+' < '+hoy);
...
function processError(data, status) {
    navigator.notification.confirm(
            'Lo sentimos no se pudo realizar la operación, por favor inténtalo nuevamente.', // message
            onConfirmOK, 
            'Atención!!!', 
            'Ok'  
        );
