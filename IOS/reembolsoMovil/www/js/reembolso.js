...
function msjDepo(){
    if(msjVarDepo == 0 ){
        msjVarDepo = 1;
        Alert('Tanto los depósitos en cuenta corriente como los vales vista se harán a nombre del titular del contrato de salud.');
    }
}
function capturePhoto1Click() {
    navigator.notification.confirm(
            'Es muy importante que la imagen tomada sea legible. En caso contrario Isapre Consalud tendrá que tomar medidas adicionales para cursar tu solicitud.', 
            onConfirm, 
            'Atención!!!', 
            'Ok'  
        );
             ...
             if(cantidadDeFotos==2){
                if(($("#smallImage1").attr("src")!='')&&($("#smallImage2").attr("src")!='')){
                    ...
                    if($("#smallImage1").attr("src")!=''){
                        Alert('Tienes que sacar la foto de la orden médica');
                    }else{
                        Alert('Tienes que sacar la foto de la boleta');
                    }
    ...
    $.ajax({
        ...
        success: function (data, status, req){
            if (status == "success"){
                ...
                if(contador>0){
                    ...
                    $.mobile.hidePageLoadingMsg(); 
                    Alert('El servicio no responde, intentelo más tarde. ');
                }
            }
        ...
        error: function(){
                    navigator.notification.alert(
                        'No se pudo obtener los beneficiarios, por favor inténtalo nuevamente.',  // message
...
function getExcedentes(){
    $.mobile.showPageLoadingMsg();
        ...
        $.ajax({
            ...
            error: function(){
                        navigator.notification.alert(
                            'No se pudo obtener los excedentes, por favor inténtalo nuevamente.',  // message
...
function getInstitucionesBancarias(selecto){
  ...
  <soap:Body> \n\
    ...
    $.ajax({
        ...
        error: function(){
                    navigator.notification.alert(
                        'No se pudo obtener las instituciones bancarias, por favor inténtalo nuevamente.',  // message
...
function ValidarDuplicidad(){
        ...
        $.ajax({
            ...
            success: function (data, status, req){
                ...
                if (status == "success"){
                    ...
                    if( estado == 'false'){
                        ...
                        if( estado == 'true'){
                            if($(req.responseXML).find('ErrorMessage').text() == "Ha ocurrido un problema al ingresar el reembolso, intentelo mas tarde. (1)"){
            ...
            error: function(){
                        navigator.notification.alert(
                            'No se pudo validar la boleta, por favor inténtalo nuevamente.',  // message
    ...
    if($('#excedentes:checked').val()){
        ...
        <soap:Body> \n\
          <RegistrarReembolsoEnWorkflow xmlns="Consalud.DMZWebCliente.DMZ.Servicios"> \n\
            ...
            <NumeroBoleta>'+$('#numeroBoleta').val()+'</NumeroBoleta>\n\
            <FechaBoleta>'+$('#fechaBoleta').val()+'</FechaBoleta>\n\
            <RutPrestador>'+rutPrestador[0]+'</RutPrestador>\n\
        ...
        $.ajax({
            ...
            error: function(){
                        navigator.notification.alert(
                            'No se pudo registrar el reembolso, por favor inténtalo nuevamente.',  // message
        ...
        success: function (data, status, req){
            ...
            if (status == "success"){
                ...
                if( $(req.responseXML).find('Data').text() == 'true'){
                    ...
                    Alert('No se pudo subir las imagenes, Intentelo nuevamente');
                    $.mobile.hidePageLoadingMsg();
                    $(".contenido").css("display", "block");
                }
            }
