    ...
    if ([target hasPrefix:kCDVAssetsLibraryPrefix]) {
        ...
        ALAssetsLibraryAssetForURLResultBlock resultBlock = ^(ALAsset* asset) {
            if (asset) {
                ...
                Byte* buffer = (Byte*)malloc((unsigned)[assetRepresentation size]);
                NSUInteger bufferSize = (NSUInteger)[assetRepresentation getBytes:buffer fromOffset:0.0 length:	(NSUInteger)[assetRepresentation size] error:nil];
